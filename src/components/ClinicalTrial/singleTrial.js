...
function SingleTrial() {
  ...
  function getStatus(props) {
    if (props.status === "active") {
      let dayLeft = Date.parse(trial.end_date) - Date.now();
      dayLeft = Math.floor(dayLeft / (1000 * 3600 * 24));
      return "active | left " + dayLeft + " day" + (dayLeft > 1 ? "s" : "");
    }
    return props.status;
  }
    ...
    <div className="bd-example">
      <div className="card">
        ...
        <ul className="list-group list-group-flush">
          ...
          <li className="list-group-item">Status : {getStatus(trial)}</li>
